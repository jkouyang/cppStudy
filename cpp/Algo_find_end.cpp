/*find_end()——查找序列中与子序列最后一次匹配的位置
*
	函数形式：
		1.iterator find_end(beg1, end1, beg2, end2);
		查找[beg1,end1)区间内与[beg2，end2)序列匹配的序列，
		指向最后一次匹配序列的第一个元素，并返回该元素的迭代器，找不到返回结束迭代器end1()
...
void testFindEnd() {
	...
	//查找[beg1, end1)区间内与[beg2，end2)序列匹配的序列，
	//指向最后一次匹配序列的第一个元素，并返回该元素的迭代器，找不到返回结束迭代器end1()
	vector<int>::iterator p1 = find_end(vect1.begin(), vect1.end(), vect2.begin(), vect2.end());
	if (p1 != vect1.end()) {
	...
	//查找[beg1, end1)区间内与[beg2，end2)序列满足_Pred()关系匹配的序列，
	//指向最后一次匹配序列的第一个元素，并返回该元素的迭代器，找不到返回结束迭代器end1()
	vector<int>::iterator p2 = find_end(vect1.begin(), vect1.end(), vect2.begin(), vect2.end(), mySelectFindEnd());
	if (p2 != vect1.end()) {
